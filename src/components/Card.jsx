...
const Card = ({
  ...
  const modal = isOpen && (
    <Modal
      isOpen={isOpen} setIsOpen={setIsOpen} areas={areas} setAreas={setAreas}
    ...
    <div className="w-full" style={{ opacity: dimmed ? 0.3 : isDragging ? 0.3 : 1, pointerEvents: dimmed ? 'none' : 'auto' }}>
      ...
      >
        ...
        <div className="px-1.5 pt-1">
          <span
            className="font-bold rounded-full truncate inline-block max-w-full leading-tight"
            style={{
              fontSize: 'clamp(5px, 0.45vw, 8px)',
              padding: '1px 5px',
              backgroundColor: typeColor,
              color: isYellow ? '#555A5A' : 'white',
            }}
          >
            {currentMachine.Typ
        ...
        <div className="px-1.5 mt-0.5">
          <span
            className="font-bold text-[rgb(85,90,90)] truncate leading-tight block"
            style={{ fontSize: 'clamp(6px, 0.65vw, 11px)' }}
          >
            {currentMachine.kunde || '—'}
          </span>
          <span
            className="text-gray-400 font-medium truncate leading-tight block"
